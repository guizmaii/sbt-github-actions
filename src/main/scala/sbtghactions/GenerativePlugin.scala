...
s"""
    ...
    // PR branches: None means no filter (all PRs), Some(list) means filter to those branches
    val renderedPRBranches = prBranches match {
      case None => ""
      case Some(branches) => s"\n    branches: [${branches.map(wrap).mkString(", ")}]"
    }
...
on:
  pull_request:$renderedPRBranches$renderedTypes$renderedPaths
  push:
    branches: [${pushBranches.map(wrap).mkString(", ")}]$renderedTags$renderedPaths
...
"""
  ...
  val settingDefaults = Seq(
    ...
    githubWorkflowPRTargetBranches := None,
  ...
  private val generateCiContents = Def task {
    ...
    compileWorkflow(
      ...
      githubWorkflowPRTargetBranches.value.map(_.toList),
